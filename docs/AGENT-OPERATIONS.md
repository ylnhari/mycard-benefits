...
- Source and catalog agents create candidates only (see
  `CATALOG-GOVERNANCE.md`). Publishing requires one dated human approval;
  an agent may assist but may not hold a reviewer role. A second human review
  may be recommended for enhanced, ambiguous, or high-impact claims, but is
  not mandatory. No assertion reaches `active` without human-approved
  evidence.
...
- Reviewing already-admitted public evidence within the source policy.
- Drafting candidate catalog assertions and evidence records for human review.
...
mid-task, it records `deferred_quota` in the durable job ledger and stops. The
next runner resumes from `CONTINUE-HERE.md`, `TASKS.md`, and committed Git
state, not from chat memory or an assumption about what was already done.

## Coordination substrate

Before starting assigned work, an agent reads, in order: `AGENTS.md`,
`CONTINUE-HERE.md`, `PROJECT_STATUS.md`, `DECISIONS.md`, and the relevant
entries in `TASKS.md`. While working, it:
