...
def test_json_csv_and_unknown_document_are_bounded_and_count_only(tmp_path: Path) -> None:
    family = _write(tmp_path / "family.json", json.dumps({"schema_version": 1, "cards": [
    ...
    records, counts, source_counts, digest = parse_sources([
        SourceInput("family_finance", family), SourceInput("workbook", workbook),
        SourceInput("documents", docs),
    ])
    assert len(records) == 2
    assert counts.parsed == 2 and counts.needs_local_review == 1 and counts.rejected == 0
    assert source_counts == {"family_finance": 1, "workbook": 1, "documents": 1}
    assert len(digest) == 64


def test_native_family_finance_card_only_export_adapts_without_offering_guessing(tmp_path: Path) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps({"schemaVersion": 1, "cards": [{
        "id": "SYNTHETIC-ONLY-FAMILY-CARD",
        "name": "SYNTHETIC-ONLY-PRODUCT",
        "bank": "SYNTHETIC-ONLY-BANK",
        "owner": "SYNTHETIC-ONLY-OWNER",
        "type": "Credit",
        "variant": "VISA",
        "number": "SYNTHETIC-ONLY-PAN-1A1B1C1D1E1F1G1H1J1K1L1M2",
        "expiry": "2030-01-01",
        "cvv": "SYNTHETIC-ONLY-CVV",
        "pin": "SYNTHETIC-ONLY-PIN",
        "fees": 0,
        "benefits": "SYNTHETIC-ONLY-BENEFIT",
        "lounge": "SYNTHETIC-ONLY-LOUNGE",
        "loungeCriteria": "SYNTHETIC-ONLY-CRITERIA",
        "status": "closed",
    }]}))
    parsed = _parse_sources([SourceInput("family_finance", source)])
    assert parsed.counts.parsed == 1 and parsed.source_counts == {"family_finance": 1}
    fields = parsed.records[0].fields
    assert fields["product"] == "SYNTHETIC-ONLY-PRODUCT"
    assert fields["network"] == "visa" and fields["lifecycle"] == "closed"
    assert fields["expiry"] == "2030-01" and "offering_id" not in fields
    cards = _cards(parsed)
    assert len(cards) == 1 and cards[0].offering_id is None


def test_native_family_finance_optional_blank_fields_are_not_private_placeholders(tmp_path: Path) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps({"schemaVersion": 1, "cards": [{
        "id": "SYNTHETIC-ONLY-FAMILY-CARD", "name": "SYNTHETIC-ONLY-PRODUCT",
        "number": "SYNTHETIC-ONLY-PAN-1A1B1C1D1E1F1G1H1J1K1L1M2",
        "bank": "", "owner": "", "type": "Credit", "variant": "", "expiry": "",
        "cvv": "", "pin": "", "fees": 0, "benefits": "", "lounge": "",
        "loungeCriteria": "", "status": "",
    }]}))
    records, _, _, _ = parse_sources([SourceInput("family_finance", source)])
    assert records[0]["lifecycle"] == "active"
    assert "bank" not in records[0] and "network" not in records[0] and "expiry" not in records[0]


def test_native_family_finance_variant_subtype_is_metadata_not_a_card_network(tmp_path: Path) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps({"schemaVersion": 1, "cards": [{
        "id": "SYNTHETIC-ONLY-FAMILY-CARD", "name": "SYNTHETIC-ONLY-PRODUCT",
        "number": "SYNTHETIC-ONLY-PAN-1A1B1C1D1E1F1G1H1J1K1L1M2",
        "variant": "", "variantSubType": "SYNTHETIC-ONLY-VARIANT-SUBTYPE",
        "status": "closed",
    }]}))
    records, _, _, _ = parse_sources([SourceInput("family_finance", source)])
    assert records[0]["variant"] == "SYNTHETIC-ONLY-VARIANT-SUBTYPE"
    assert "network" not in records[0] and "offering_id" not in records[0]


def test_native_family_finance_legacy_master_card_network_normalizes_without_offering_guess(tmp_path: Path) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps({"schemaVersion": 1, "cards": [{
        "id": "SYNTHETIC-ONLY-FAMILY-CARD", "name": "SYNTHETIC-ONLY-PRODUCT",
        "number": "SYNTHETIC-ONLY-PAN-1A1B1C1D1E1F1G1H1J1K1L1M2", "variant": "master card",
    }]}))
    records, _, _, _ = parse_sources([SourceInput("family_finance", source)])
    assert records[0]["network"] == "mastercard"
    assert "offering_id" not in records[0]


def test_native_family_finance_rejects_unapproved_network_aliases(tmp_path: Path) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps({"schemaVersion": 1, "cards": [{
        "id": "SYNTHETIC-ONLY-FAMILY-CARD", "name": "SYNTHETIC-ONLY-PRODUCT",
        "number": "SYNTHETIC-ONLY-PAN-1A1B1C1D1E1F1G1H1J1K1L1M2", "variant": "master-card",
    }]}))
    with pytest.raises(ImportRejected):
        parse_sources([SourceInput("family_finance", source)])


@pytest.mark.parametrize("expiry", ["2030-01-02", "2028-02-29", "1999-12-31"])
def test_native_family_finance_valid_iso_expiry_keeps_only_its_month(tmp_path: Path, expiry: str) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps({"schemaVersion": 1, "cards": [{
        "id": "SYNTHETIC-ONLY-FAMILY-CARD", "name": "SYNTHETIC-ONLY-PRODUCT",
        "number": "SYNTHETIC-ONLY-PAN-1A1B1C1D1E1F1G1H1J1K1L1M2", "expiry": expiry,
    }]}))
    records, _, _, _ = parse_sources([SourceInput("family_finance", source)])
    assert records[0]["expiry"] == expiry[:7]


@pytest.mark.parametrize("expiry", ["2030-02-29", "2030-13-01", "2030-1-01", "01/2030"])
def test_native_family_finance_invalid_expiry_remains_rejected(tmp_path: Path, expiry: str) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps({"schemaVersion": 1, "cards": [{
        "id": "SYNTHETIC-ONLY-FAMILY-CARD", "name": "SYNTHETIC-ONLY-PRODUCT",
        "number": "SYNTHETIC-ONLY-PAN-1A1B1C1D1E1F1G1H1J1K1L1M2", "expiry": expiry,
    }]}))
...
@pytest.mark.parametrize("payload", [
    {"schemaVersion": 1, "cards": [], "ledgers": []},
    {"schemaVersion": 1, "cards": [{"id": "SYNTHETIC-ONLY-ID", "name": "SYNTHETIC-ONLY-NAME", "number": "SYNTHETIC-ONLY-PAN", "transactions": []}]},
    {"schemaVersion": 1, "cards": [{"id": "SYNTHETIC-ONLY-ID", "name": "SYNTHETIC-ONLY-NAME", "number": "SYNTHETIC-ONLY-PAN", "status": "active"}]},
])
def test_native_family_finance_rejects_ledger_transaction_and_unknown_status(tmp_path: Path, payload: dict[str, object]) -> None:
    source = _write(tmp_path / "family-export.json", json.dumps(payload))
    with pytest.raises(ImportRejected):
        parse_sources([SourceInput("family_finance", source)])
...
def test_receipt_is_schema_closed_and_does_not_reflect_private_values(tmp_path: Path) -> None:
    ...
    receipt = Consolidator(tmp_path / "data", b"SYNTHETIC-ONLY-KEY").run(
        [SourceInput("family_finance", source)]
    ).as_dict()
...
def test_symlink_and_formula_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ImportRejected):
        parse_sources([SourceInput("family_finance", link)])
...
def test_apply_is_one_authorized_atomic_batch_and_preserves_lkg(tmp_path: Path) -> None:
    ...
    preview = consolidator.run([SourceInput("family_finance", source)], session=session)
    ...
    receipt = consolidator.run(
        [SourceInput("family_finance", source)], apply=True, approved_digest=preview.preview_digest,
...
def test_strict_records_documents_and_csv_headers_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ImportRejected):
        parse_sources([SourceInput("family_finance", non_object)])
...
def test_compatible_cross_source_observations_merge_and_policy_limits_bind(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    parsed = _parse_sources([SourceInput("family_finance", first), SourceInput("workbook", second)])
    cards = _cards(parsed)
    assert len(cards) == 1
...
def test_apply_authorization_rejects_same_session_target_revision_change(tmp_path: Path) -> None:
    ...
    with pytest.raises((VaultConflictError, ImportRejected)):
        consolidator.run(
            [SourceInput("family_finance", source)], apply=True,
...
def test_failed_prewrite_apply_allows_corrected_source_retry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.undo()
    retry_preview = consolidator.run([SourceInput("family_finance", source)], session=session)
